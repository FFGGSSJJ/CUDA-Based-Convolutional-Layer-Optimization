/* v1
 * Optimized version of Forward-conv using ‘Tiled shared memory convolution (2 points)’
*/
...
#define TILE_WIDTH 8
__global__ void convf_shared_kernel(float *y, const float *x, const float *k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    /*
...
#define w4d(i3, i2, i1, i0) Wc[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
    ...
    float* Kshared = &shared_mem[X_tile_width * X_tile_width];
    ...
    for (int c = 0; c < C; c++) {
        // load K into the shared memory
        if (h0 < K && w0 < K) 
            Kshared[h0 * K + w0] = k4d(m, c, h0, w0);
        __syncthreads();
        ...
        for (int p = 0; p < K; p++) {
            for (int q = 0; q < K; q++) {
                if (h < H_out && w < W_out)
                    acc += Xshared[(h0+p)*X_tile_width + (w0+q)] * Kshared[p*K + q];   // 0.6954
                    //acc += x4d(n, c, h+p, w+q) * Kshared[p*K + q];      // 0.2233
            }
        }
        __syncthreads();
    }
...
__host__ void GPUInterface::conv_forward_gpu(float *device_y, const float *device_x, const float *device_k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    dim3 GridDim(B, M, Z);
    // dynamic shared memory is needed
    convf_shared_kernel<<<GridDim, BlockDim, sizeof(float)*((TILE_WIDTH + K - 1)*(TILE_WIDTH + K - 1) + K*K)>>>(device_y, device_x, device_k, B, M, C, H, W, K);
}
