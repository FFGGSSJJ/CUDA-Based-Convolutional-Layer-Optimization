...
// C = y = k*x is the output matrix with size M*(H_out*W_out)
// B = x is input matrix with size (C*K*K)*(H_out*W_out)
...
//   __shared__ float A_ds[TILE_WIDTH][TILE_WIDTH];
  ...
  for (int i = 0; i < k; i++) {
    ...
    if ((i*TILE_WIDTH + ty) < numBRows && Col < numBColumns)
      B_ds[ty][tx] = B[(i*TILE_WIDTH + ty)*numBColumns + Col];
    else  B_ds[ty][tx] = 0;
    __syncthreads();
    for (int j = 0; j < TILE_WIDTH; j++){
      //Cval += A_ds[ty][j]*B_ds[j][tx];
      Cval += Kc[Row*numAColumns + i*TILE_WIDTH + j] * B_ds[j][tx];
    }
    __syncthreads();
...
// x_unroll is of size (C*K*K)*(H_out*W_out)
...
// x_unroll is output matrix with size (C*K*K)*(H_out*W_out)
// x is input with size C*H*W
void unroll_gpu(float *x_unroll, const float *x, const int C, const int H, const int W, const int K)
{
    ...
    unroll_kernel<<<block_num, BLOCK_SIZE>>>(x_unroll, x, C, H, W, K);
}
...
void gemm(float *y, const float *x, const float *k, const int M, const int C, const int H, const int W, const int K)
{
    ...
    dim3 dimGrid(ceil(1.0*numyColumns/TILE_WIDTH), ceil(1.0*numyRows/TILE_WIDTH), 1);
